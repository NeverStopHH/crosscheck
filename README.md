...
- [docs/FOREIGN-MODELS.md](docs/FOREIGN-MODELS.md) — running the Tier-1 inference on a model that is not Claude: the exact `CROSSCHECK_SUMMARIZER_CMD` contract, what is guaranteed, and what is not
...
- **Claude Code** — full Tier-1 capture, both wings, and the reference rung every other connector's manifest is read against. Drafts stay `capture_mode=auto`, `provenance=derived`, confidence-capped at 0.5, and are never proactively injected to teammates — pointers only, until the author's own agent promotes them via `review_draft`.
- **Cursor** — Tier-1 capture too, since 2026-08-28: `beforeSubmitPrompt` fires the derived intent, `stop` and `postToolUse` pay the ghost debt, and `stop` runs the same gate over the transcript Cursor's own payload points at (`transcript_path`; `state.vscdb` stays off the table). The summarizer rung is **reduced**, and for one documented reason: Cursor documents where the transcript is and never what is in it, so the reader tries two shapes and BOOKS a tail it cannot read as its own outcome instead of guessing. `crosscheck doctor` prints each rung, and each thing Cursor cannot have, as a sentence — see [packages/connector-cursor/README.md](packages/connector-cursor/README.md).
- **ACP agents** (Zed, Gemini CLI, Goose, anything speaking ACP) — Tier-1 capture too, since 2026-08-28, and the proxy observes the WIRE, not a transcript file. `session/prompt` fires the derived intent and pays the ghost debt; its response is the turn boundary the gate runs on. The summarizer rung is **reduced** for a reason the protocol decides rather than a vendor: the slice is only what the wire happens to carry (the agent's message chunks, a failed tool call's output, terminal output tails), so an agent doing its work outside ACP's `terminal/*` methods yields a prose-only slice — `crosscheck acp-report` prints which sources YOUR agent actually emits. In one way it is the strongest of the three: the proxy is a long-lived parent, so the slice reaches the model on stdin and no slice artifact is ever written to disk. `crosscheck doctor` prints each rung and each refusal as a sentence — see [packages/connector-acp/README.md](packages/connector-acp/README.md).
...
- **It does not have to be Claude.** `CROSSCHECK_SUMMARIZER_CMD=/path/to/your-wrapper` replaces the binary WHOLESALE — spawned with no arguments, slice on stdin, one answer on stdout, exit 0 — and the parse, the gates, the derived cap and the booked outcomes are identical whatever answers. A working example wrapper is [`docs/examples/foreign-model-wrapper.sh`](docs/examples/foreign-model-wrapper.sh) and the contract is [docs/FOREIGN-MODELS.md](docs/FOREIGN-MODELS.md), which is equally explicit about what is NOT guaranteed: **no vendor's model has been tested**, the corpus behind the contract test is authored rather than recorded, and one variable serves four tasks that want four different answers.
