...
> Die fehlerhafte Team-Zuordnung entsteht in der Competition Resolution.

Agent B untersucht parallel ein ähnliches Verhalten und erkennt:

> Die Competition Resolution ist zwar der sichtbare Fehlerpunkt, die eigentliche Root Cause liegt aber bereits in der fehlerhaften Entity Resolution beziehungsweise im fehlenden Mapping beim Import.

Heute würden diese Erkenntnisse möglicherweise getrennt voneinander bleiben. Agent A würde einen Fix in der Competition Resolution bauen, obwohl Agent B bereits erkannt hat, dass dieser Fix lediglich das Symptom behandelt.
...
**Beobachtung** — Spieler oder Teams erscheinen ohne korrekte Competition-Zuordnung.

**Erste Hypothese** — Die Competition Resolution schlägt bei bestimmten Teams fehl.

**Evidenz** — Mehrere Teams werden als „unknown competition" gespeichert.

**Erweiterte Hypothese** — Die Competition Resolution erhält bereits unvollständige oder falsch aufgelöste Entities.

**Tiefere Root Cause** — Beim Import werden Transfermarkt- und StatsBomb-Entities nicht konsistent zusammengeführt.

**Verworfen** — Der Transfermarkt-Extractor selbst klickt oder lädt die falschen Teams.

**Nächster Test** — Prüfung der Entity IDs direkt nach dem Ingestion-Schritt und vor der Competition Resolution.
