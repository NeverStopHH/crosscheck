...
**Shipped, and what it turned out to require.** Matching was the easy half; DELIVERY was the work. Three tiers feed the surface — a shared error fingerprint (content identity, and the only one allowed to travel between repos), a shared file inside one repo, and an overlap with what this session SAID it was doing — and the tier decides what the line may say: identity earns the recorded root cause, the weaker two earn a pointer (DESIGN.md §4). It reaches the reader at two moments, and the second one is why the feature is worth having: at SessionStart, and again the instant a tool fails with a fingerprint somebody already diagnosed, which on a long agent turn is an hour earlier than the next briefing. That second moment also exposed a defect the audit had not caught — Claude Code fires `PostToolUse` only on SUCCESS and routes failures to `PostToolUseFailure`, which the connector never registered, so on its main host this product had been capturing no error fingerprints at all. Finally, the surface counts itself: `solved-tree pointers: N shown, M opened`, with a `doctor` WARN — ending in the `get_diagnosis <id>` that settles it — when the pointers are shown and never opened. The label is the rows' own name rather than the feature's: the delivery ledger has no source column yet, so the count includes ordinary teammate pointers at trees somebody later solved.
