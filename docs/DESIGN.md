...
- **PreToolUse tripwire** (v0.5): file+symbol overlap with an *active* teammate session → `permissionDecision: "ask"` with reason, and the same facts plus the `get_diagnosis` id as `additionalContext` so the model is briefed too (trial finding #25). Escalation ladder: silence → notice → ask. Never hard-deny — with one documented host behaviour: a headless `claude -p` / Agent-SDK session cannot prompt, and Claude Code turns a hook ask into a one-shot deny of that tool call with the reason delivered to the model (settled empirically on 2.1.237; the next identical edit passes, one ask per file per session). A headless marker exists but cannot be trusted (`CLAUDE_CODE_ENTRYPOINT` reads `sdk-cli` only when the caller left it unset, and an orchestration subagent inherits its parent's interactive value), so `CROSSCHECK_TRIPWIRE=notice` selects the ladder's `notice` rung explicitly (additionalContext only, no decision) for orchestration/CI sessions; `status`/`doctor` print the mode. The file's repo-relative id resolves against the root that governs the TOUCHED file — a linked worktree of the same repo, not only the session's checkout, and not the hook cwd's worktree either when the file lives in a third one (trial finding #17).
