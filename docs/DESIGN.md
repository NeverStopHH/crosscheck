...
- **Echo-loop exclusion** (judge-found, mandatory): content injected as teammate hints is marked and excluded from summarizer capture, so Can's hypothesis can never be re-captured as Nick's independent observation.
...
- **Statusline** = human-facing presence ("Can → match.service.ts · debugging starts column") **plus a `last sync 2m ago` health indicator** — fail-open must never mean silently dead.
