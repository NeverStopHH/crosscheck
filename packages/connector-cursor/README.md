...
**Doctor**: `crosscheck doctor` gains a cursor section — hooks registered (all seven, none `failClosed`), launcher health (a bare `crosscheck` on PATH is resolved *and* executed via `--version`; an absolute-path launcher is checked for existence and package-runner caches, not executed), mcp entry owned, last-observed `cursor_version` ≥ 1.7, and the **contract-drift counters**.
