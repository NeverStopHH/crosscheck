...
| `captureTouchedFiles` | **The capture entry point.** `resolveTouchedRoots` pre-pass (the file's OWN worktree root, the per-session root cache, the foreign-vs-outside drop split) → `captureFileTargets` with the resolved `resolveRoot`. The only place in the repo that builds the raw call. Pair it with `withCaptureBookkeeping` for the counters `status`/`doctor` read; session state stays the CALLER's: follow with `updateSessionState(withSeenTargets)` — hosts batch state writes differently. | `connector-claude/src/hooks/post-tool-use.ts`, `connector-cursor/src/handlers/file-edit.ts`, `connector-acp/src/capture/engine.ts` |
| `captureFileTargets` | `toRepoRelative` → `isDenied` → seen-set filter → `containsSecret` → spool `targetRecord`s (cap `MAX_TARGETS_PER_INVOCATION`). **Internal — do not call directly:** its `resolveRoot` hook is OPTIONAL, and an absent one resolves every touch against the session checkout — the pre-#17 behaviour that lost every edit made in a linked worktree with no counter (finding H1: 371 worktree edits → 0 targets). Go through `captureTouchedFiles`. | `flows/capture-touched-files.ts` only |
