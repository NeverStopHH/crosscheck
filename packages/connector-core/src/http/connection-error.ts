...
export const describeConnectionFailure = (
  ...
  switch (cause) {
    ...
    case "unknown":
      // THE ONE BRANCH THAT PRINTS SOMEBODY ELSE'S WORDS. Every case above
      // is a renderer-owned sentence plus the hub URL the developer
      // configured themselves; this one relays whatever the far side chose to
      // say, unbounded and uninspected. It reaches `crosscheck doctor`,
      // `login` and `conference` — three commands whose registrations say
      // they interpolate nothing untrusted — so the cap and the control-strip
      // belong HERE, where all three meet, rather than three times over.
