...
const diagnosis = (overrides: Partial<Diagnosis> = {}): Diagnosis => ({
  // 03 makes `coverage` required on a Diagnosis, so every surface saying what
  // a team knows also says how far it looked. This fixture is about the intent
  // chain and measured no coverage; unknown is the honest value.
