...
import { MAX_COMMIT_EVIDENCE_AUTHORS } from "@crosscheck/schema";
...
/** git's field separator for the log format below — never appears in emails. */
 ...
 *
 * This is the Tier-0 evidence absence detection rests on (roadmap item 3): a
 ...
 * else moves forward; author date stays with the person who did the work.
 *
...
export const collectCommitEvidence = async (
  ...
  const entries = output
    ...
    .filter((entry) => entry.authoredAtMs >= windowStartMs);
  if (entries.length === 0) {
    return null;
  }
