...
const NO_COST: SummarizerCost = { sessions: 0, fires: 0, estimatedTokens: 0 };
...
/** The one spelling of the cost fact both CLI surfaces print. */
export const formatSummarizerCost = (cost: SummarizerCost): string => {
  ...
  return (
    `${String(cost.fires)} runs, ~${String(cost.estimatedTokens)} tokens ` +
    `(estimate) across ${sessionsPart}`
  );
};
