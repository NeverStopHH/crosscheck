...
- **A one-sentence session intent** — either declared by the agent through the `set_intent` MCP tool (confidence 1, secret-scanned and refused rather than uploaded on a hit), or derived: on the first substantive prompt of the session the connector spawns the summarizer's detached worker and asks Haiku for ONE sentence, third person, of what the session is trying to accomplish (or NONE). The sentence is capped at 200 characters, secret-scanned (drop, never redact), echo-checked against delivered teammate hints, and uploaded as a `(derived)` intent with confidence 0.4. **The raw prompt itself never leaves the machine**: the hook parks it in a 0600 file, the worker hands it to the model's stdin and unlinks it, whatever happens.

- **A ghost check** — at most ONE per session, and only when the hub has already found somebody: `GET /api/ghost-checks` reports the teammates whose live work overlaps yours (a shared error fingerprint, two shared files or symbols, or three distinct words of your own stated intent), and only then does the connector spawn the same detached worker over the two intent sentences plus that teammate's DECLARED claims. What comes back is one sentence naming where the two plans would collide, or NONE; it is stored as YOUR OWN unreviewed draft (`derived`, confidence 0.4, `review_draft` promotes it) and shown to nobody else. The draft says whose plan it collides with and carries the `get_diagnosis` id of their tree, because the model is never told who the other session belongs to — that half is attached afterwards, from the same overlap row. **No transcript, no prompt and no machine-written draft of theirs ever reaches that call**, and a sentence that merely repeats a claim it was shown is dropped rather than published.
