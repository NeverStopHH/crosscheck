 ...
 *
 * HISTORICAL, and not re-derivable from this tree. The previous version of this
 * constant claimed to be stated independently so that "a test that borrowed the
 * implementation's own pattern would agree with it however it was weakened". It
 * was a character-for-character copy of INVISIBLE_PATTERN in
 * briefing/sanitize.ts — the two literals diffed clean — so it could not
 * disagree with the implementation about anything, including the invisible
 * characters both of them let through. Neither that constant nor
 * INVISIBLE_PATTERN survives here, so no command re-measures the overlap and
 * no figure for it is quoted.
 *
 * What replaces it asks Unicode instead: is this character's general category
 * one of the classes a briefing line may never carry? The runtime's own
 * character database answers, and the sanitizer has no say in it. Weakening the
 * implementation's class — dropping \p{Cf} from it — therefore turns this file
 * red, which ci.yml's mutation job re-proves on every pull request.
 */
const INVISIBLE_BY_CATEGORY = /[\p{Cc}\p{Cf}\p{Zl}\p{Zp}]/u;
/**
 ...
 */
const ZERO_WIDTH_MARKS = /[͏᠋-᠍᠏︀-️\u{E0100}-\u{E01EF}]/u;
/**
 ...
 *
 * The tag block at its start is a MIX: U+E0001 and U+E0020-U+E007F are Cf and
 * so the `\p{Cf}` class above already sees them, but U+E0000 and
 * U+E0002-U+E001F are Cn and it does not. Those Cn members are the only reason
 * the sanitizer spells the range out, and `tag-characters-unassigned` in the
 * corpus is built from them.
 */
const PLANE_14 = /[\u{E0000}-\u{E0FFF}]/u;
/** Backtick, angle brackets, backslash: the renderer never emits one. */
...
const countOf = (line: string, needle: string): number =>
  line.split(needle).length - 1;

/** Characters no line may carry, whichever section it belongs to. */
const assertCharacters = (line: string, where: string): void => {
  ...
  // The frame is the renderer's: opened and closed exactly by it, never
  // opened, closed or nested by whoever wrote the text inside.
