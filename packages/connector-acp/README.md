...
The ACP transparent proxy — Blocks 3–4 of [docs/adapters/DESIGN-agent-agnostic.md](../../docs/adapters/DESIGN-agent-agnostic.md): `crosscheck acp -- <agent command…>` wraps any [ACP](https://agentclientprotocol.com) agent as a byte-transparent stdio pipe, and (since Block 4) captures Tier-0 signals from a parsed COPY of the wire. One binary covers every ACP client (Zed, JetBrains, Neovim, Emacs) × every ACP agent, because the client simply runs crosscheck's binary as the agent command.
...
- **Tier-0 capture (Block 4, design §2.4)**: from the observer's parsed copies — never the forward path — the proxy maps the wire onto the crosscheck model through the core kit flows:
  ...
  - `session/load`/`resume` → idempotent re-register at REQUEST time, so replayed history is captured safely (server-side natural-key dedup absorbs the repeats — pinned);
  ...
  Everything is fail-open: a session in a directory with no repo or no crosscheck config is a pure pipe that talks to nobody; the capture queue is byte-capped (drops counted); a capture crash is a counter and a log line. JSON-RPC error responses on captured methods are counted, never recorded. Not captured, deliberately: prompt/response content, `rawInput`, diff old/new text, permission outcomes.
- **Capture-quality measurement** (design §6.1): `crosscheck acp-report <record-file>` analyzes an `acp --record` transcript and reports which signals the agent actually emitted — tool calls, locations, diff paths, failures with output, terminals, fs writes, plus recording honesty (gaps/oversized/unparseable). This is the tool that decides each agent's documented capture level; run it against a real `--record` of Gemini CLI / cursor-agent / Goose before writing that agent's doc.
...
- **Injection is Block 5.** No message is ever modified: no `mcpServers` append, no prompt-block append, no `--no-inject` flag (it arrives with the feature it gates). The two §2.5 write points will re-serialize exactly two message kinds; everything else stays verbatim forever. Until then this package renders NOTHING into any agent's context — it has no render surfaces to register (§4.4), and the registry meta-test verifies that.
...
- **`crosscheck status`/`doctor` surfacing of drop counters** still waits; today the counters live in the log summary line (`capture sessions=… targets=… fingerprints=…`).
...
| `src/capture/engine.ts` | the §2.4 mapping over the core kit flows — serialized, byte-capped, fail-open |
