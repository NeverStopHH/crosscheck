...
/** Nick's wc_01 + clm_01, plus Can's own session and claim clm_can. */
...
describe("POST /api/records", () => {
  ...
  test("rejects records whose producer session belongs to another developer", async () => {
    // Arrange: Nick owns the registered session, Can sends with Nick's session id
    const { harness } = await createHarnessWithSession();
  ...
  test("ignores session and hint kinds with an explanatory issue", async () => {
    ...
    const hintBody = {
      ...
      refKind: "claim",
      refId: "clm_01",
      renderedText: "Can rejected the cache theory 2h ago.",
      trust: { authorName: "Can", ageSeconds: 7200 },
  ...
  test("does not dedup the same body from a different developer", async () => {
    // Arrange: Can has his own session and extends Nick's work context
    const { harness, developer } = await createHarnessWithSession();
    await postRecords(harness, developer, {
      records: [
        recordEnvelope("work_context", validWorkContextBody()),
    ...
    const { data } = await postRecords(
      ...
      recordEnvelope(
        "claim",
        validClaimBody({ id: "clm_can", authorSessionId: SECOND_SESSION_ID }),
        { sessionId: SECOND_SESSION_ID },
      ),
    );
    ...
    expect(data?.results[0]?.status).toBe("accepted");
    expect(data?.results[0]?.id).toBe("clm_can");
  ...
  test("rejects a claim id already used by another developer", async () => {
    // Arrange: Nick owns clm_01, Can has his own session
    const { harness, second } = await seedCrossAuthorClaims();

    // Act: Can reuses Nick's claim id with a different body
    const { data } = await postRecords(
      harness,
      second,
      recordEnvelope(
  ...
  test("rejects a cross-author supersedes edge", async () => {
    // Arrange: Can's clm_can and Nick's clm_01 in the same tree
    const { harness, second } = await seedCrossAuthorClaims();

    // Act: Can tries to supersede Nick's claim
    const { data } = await postRecords(
      harness,
      second,
