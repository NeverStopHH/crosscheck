...
describe("doctor prints the order failures only the hub can see", () => {
  ...
  test.each([
    ...
    [
      ...
      "WARN  skeleton retention  the last 2 sweep passes failed, and a failed pass deletes nothing; the sweep has not finished a cycle over this hub's sessions since the hub started (last pass 2026-09-24T06:15:00.000Z)",
    ],
    [
    ...
    [
      "a newer hub's report missing a field this CLI needs, with nothing to warn about",
      "interim",
      { heldBy: [], aged: 1 },
      "PASS  skeleton retention  the hub reports what it keeps in a form this crosscheck cannot read — upgrade the CLI to see it",
    ],
    [
      "a newer hub's report that only ADDS a field, which is still read",
      "interim",
      {
        windowDays: 30,
        heldBy: [],
    ...
    [
      ...
      "PASS  skeleton retention  nothing is swept in this mode, so nothing is judged either",
    ],
