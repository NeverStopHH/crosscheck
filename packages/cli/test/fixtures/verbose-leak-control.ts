 ...
 * applies. If THIS process's stderr does not carry the verbose header line
 * (the raw marker on bun ≤ 1.3.x, `Bearer [redacted]` on 1.4+, where the
 * runtime redacts values), the fixture is not triggering verbose logging on
 * the running bun and the main assertion would be vacuous — the control
